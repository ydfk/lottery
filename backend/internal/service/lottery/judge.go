...
func JudgeNumbers(code string, redNumbers string, blueNumbers string, draw model.DrawResult, prizeMap map[string]float64) PrizeResult {
	switch code {
	case "dlt":
		return judgeDLT(redNumbers, blueNumbers, draw, prizeMap)
	default:
...
func judgeDLT(redNumbers string, blueNumbers string, draw model.DrawResult, prizeMap map[string]float64) PrizeResult {
	redHit := countHit(parseCSVNumbers(redNumbers), parseCSVNumbers(draw.RedNumbers))
	blueHit := countHit(parseCSVNumbers(blueNumbers), parseCSVNumbers(draw.BlueNumbers))
	...
	result.IsWinning = true
	result.PrizeName = prizeName
	result.PrizeAmount = resolvePrizeAmount("dlt", prizeName, prizeMap)
	return result
}
...
func resolveDLTPrizeName(redHit int, blueHit int) string {
	...
	case (redHit == 3 && blueHit == 1) || (redHit == 2 && blueHit == 2):
		return "八等奖"
	case (redHit == 3 && blueHit == 0) || (redHit == 2 && blueHit == 1) || (redHit == 1 && blueHit == 2) || blueHit == 2:
		return "九等奖"
	default:
		return ""
	}
